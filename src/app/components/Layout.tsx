...
export default function Layout() {
  ...
  // Root route (/) should be full-screen without sidebar/header
  const isRoot = location.pathname === "/";

  return (
    <div className="min-h-screen bg-gray-50">
      {/* Sidebar backdrop - hidden on root */}
      {!isRoot && sidebarOpen && (
        <div
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="flex items-center justify-between h-16 px-6 border-b border-gray-200">
              <div className="flex items-center space-x-3">
                ...
                <h1 className="text-lg font-semibold text-gray-900">KH Tasks</h1>
              </div>
              <button
                onClick={() => setSidebarOpen(false)}
            ...
            <div className="px-6 py-4 border-t border-gray-200">
              <p className="text-xs text-gray-500">KH Tasks System v1.0</p>
            </div>
          </div>
        </aside>
      )}
      ...
      {/* Main content */}
      <div>
        {/* Header with menu button - hidden on root */}
        {!isRoot && (
        ...
        {/* Page content - no padding on root for fullscreen */}
        <main className={isRoot ? "min-h-screen p-0" : "min-h-screen p-6 lg:p-8"}>
          <Outlet />
        </main>
      </div>
    </div>
